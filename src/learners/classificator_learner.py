...
class ClassificatorLearner(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.train_metrics.update(torch.sigmoid(y_) if self.cfg.multilabel else torch.argmax(y_, dim=1), y)
        ret = {'loss': loss}
        if self.return_train_ouput:
            ret['output'] = torch.sigmoid(y_) if self.cfg.multilabel else torch.softmax(y_, dim=1)
        return ret
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y_ = self.forward(x)
        loss = self.loss_f(y_, y.float() if self.cfg.multilabel else y.argmax(dim=1))
        self.val_metrics.update(torch.sigmoid(y_) if self.cfg.multilabel else torch.argmax(y_, dim=1), y)
